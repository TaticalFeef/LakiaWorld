...
/**
  ...
  */
/datum/component/Destroyed(force=FALSE, silent=FALSE)
	if(!force && parent)
		_RemoveFromParent()
	if(!silent)
...
/datum/proc/RegisterSignal(datum/target, sig_type_or_types, proctype, override = FALSE)
	if(ZDELETED(src) || ZDELETED(target))
		return
...
/datum/proc/UnregisterSignal(datum/target, sig_type_or_types)
	...
	for(var/sig in sig_type_or_types)
		switch(length(lookup[sig]))
			...
			if(1)
				world << "[target] ([target.type]) somehow has single length list inside comp_lookup"
...
/datum/proc/GetComponent(datum/component/c_type)
	...
	if(initial(c_type.dupe_mode) == COMPONENT_DUPE_ALLOWED || initial(c_type.dupe_mode) == COMPONENT_DUPE_SELECTIVE)
		world << "GetComponent was called to get a component of which multiple copies could be on an object. This can easily break and should be changed. Type: \[[c_type]\]"
...
/datum/proc/AddComponent(new_type, ...)
	...
	if(!old_comp && !ZDELETED(new_comp)) // Nothing related to duplicate components happened and the new component is healthy
